...
public class ProductRepositoryImp implements ProductoRepository{
    ...
    public Producto crear(Producto producto){
        try(Connection conn = sql2o.open()){
            String sql = "INSERT INTO producto (id,nombre, categoria, precio, fecha_vencimiento)" +
    ...
    public List<Producto> getAll() {
        try(Connection conn = sql2o.open()){
            return conn.createQuery("select * from producto order by nombre ASC")
    ...
    public List<Producto> show(String id) {
        try(Connection conn = sql2o.open()){
            return conn.createQuery("select * from producto where id = :id ")
                    .addParameter("id",id)
                    .executeAndFetch(Producto.class);
        } catch (Exception e) {
            System.out.println(e.getMessage());
    ...
    public List<Producto> showLessThan(Integer precio) {
        try(Connection conn = sql2o.open()){
            return conn.createQuery("select * from producto where precio< :precio ::int")
    ...
    public List<Producto> showByCategorie(int categoria) {
        try(Connection conn = sql2o.open()){
            return conn.createQuery("select * from producto where categoria = :categoria")
    ...
    public void delete(String id) {
        try(Connection conn = sql2o.open()){
            conn.createQuery("DELETE from producto where id = :id")
                    .addParameter("id",id)
                    .executeUpdate();
        }catch (Exception e) {
            System.out.println(e.getMessage());
    ...
    public String update(Producto producto, String id){
        try(Connection conn = sql2o.open()){
            String updateSql = "update producto set nombre=:nombre, categoria= :categoria, precio=:precio, fecha_vencimiento=:fecha WHERE id=:id";
